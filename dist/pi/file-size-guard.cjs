...
function loadLimits(cwd) {
  try {
    const parsed = limitsFrom(JSON.parse((0, import_node_fs.readFileSync)(import_node_path.default.join(cwd, CONFIG_REL), "utf8")));
...
function scanAll(root, cwd) {
  ...
  for (const p of allFiles(root)) {
    const entry = classify(import_node_path.default.join(root, p), cwd, ex, limits);
    if (!entry) continue;
    counts[entry.tier]++;
    flagged.push(entry);
  }
